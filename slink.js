...
(() => {
	...
	function runSlink() {
		...
		const existingOverlay = document.getElementById("slink-overlay");
		if (existingOverlay) {
		...
		if (previousSite && previousSite !== currentSite) {
			...
			createOverlay(previousSite, currentSite);
	...
	function createOverlay(previousSite, currentSite) {
	...
	function saveCurrentSite() {
		...
		const isCurrentNavSuitable =
			(navHistory.length > 0 &&
				navHistory[navHistory.length - 1] !== currentSite) ||
			navHistory.length === 0;

		const isCurrentURLSuitable =
			(urlHistory.length > 0 &&
	...
	function goBack() {
		...
		if (urlHistory.length > 0 && navHistory.length > 0) {
			...
			const defaultURL = window.location.origin + "/SomeProot/"; // Default URL to go to if no history (Personal Website)
			const historyIsLost =
	...
	if (window.Turbo) {
		document.addEventListener("turbo:load", runSlink);
	} else {
		document.addEventListener("DOMContentLoaded", runSlink);
	}
})();
